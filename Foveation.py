#CODE FOUND AT https://stackoverflow.com/questions/28910285/image-foveation-in-python
#Written by user rayryeng on Mar 7 '15
...
class Foveation:
    ...
    def foveate(self, img):
        height = self.blur_radius*4 # Get the dimensions
        width = self.blur_radius*4

        # Define mask
        mask = 255*np.ones(img.shape, dtype='uint8')

        # Draw circle at x = 100, y = 70 of radius 25 and fill this in with 0
        cv2.circle(mask, (int(width/2),int(height/2)), self.blur_radius, 0, -1)    
        ...
        # cv2.imwrite('quarterwidth.png', filtered)
        # cv2.waitKey(0)
